...
<form method="POST" action="/projects/${p.ProjectID}/savings/add">
  <div class="card mb-3"><div class="card-header"><strong>Savings Information</strong></div><div class="card-body"><div class="row g-3">
    <div class="col-md-4"><label class="form-label">Spend ($) *</label><input type="number" step="0.01" class="form-control" name="spend" id="spend" required onchange="calcSavings()"></div>
    ...
    <div class="col-md-4"><label class="form-label" id="svLabel">Savings Amount ($)</label><input type="number" step="0.01" class="form-control" name="savingsAmountValue" id="svValue" onchange="calcSavings()"></div>
    ...
    <div class="col-md-4"><label class="form-label">COI ($)</label><input type="number" step="0.01" class="form-control" name="coi" id="coi" value="0" onchange="calcNet()"></div>
    <div class="col-md-4"><label class="form-label">NET ($)</label><input type="number" step="0.01" class="form-control" id="net" readonly></div>
    <div class="col-md-4"><label class="form-label">Implementation Date *</label><input type="date" class="form-control" name="implementationDate" required></div>
    <div class="col-md-4"><label class="form-label">One Time Savings</label><select class="form-select" name="oneTimeSavings"><option value="No">No</option><option value="Yes">Yes</option></select></div>
...
function calcSavings(){var sp=parseFloat(document.getElementById('spend').value)||0,t=document.getElementById('savingsAmountType').value,v=parseFloat(document.getElementById('svValue').value)||0,s=t==='Percentage'?sp*(v/100):v;document.getElementById('savings').value=s.toFixed(2);calcNet();}
function calcNet(){var s=parseFloat(document.getElementById('savings').value)||0,c=parseFloat(document.getElementById('coi').value)||0;document.getElementById('net').value=(s-c).toFixed(2);}
</script>`);
};
...
exports.spendDetails = (data) => {
  const { project: p, details, savings, totalSpend, totalSavings } = data;
  const detailRows = details.map(d => `<tr>
    ...
    ${p.SCSCommodity_TypeID === 2 && hasPerm(data, 'LinkSSLC') && d.SSLCs_ID ? `<td><form method="POST" action="/spend/${p.ProjectID}/sslc/${d.SSLCs_ID}/unlink" onsubmit="return confirm('Unlink?')"><button class="btn btn-sm btn-outline-danger"><i class="bi bi-x-circle"></i></button></form></td>` : ''}
  </tr>`).join('');
  ...
  return page(data, `
<div class="d-flex justify-content-between align-items-center mb-3">
  <div><h3><i class="bi bi-currency-dollar"></i> Spend &amp; Savings Details</h3><span class="text-muted">${e(p.ProjectName)}</span> <span class="badge bg-${p.SCSCommodity_TypeID === 2 ? 'info' : 'secondary'}">${e(p.Commodity_TypeName)}</span></div>
...
<div class="row mb-3">
  <div class="col-md-3"><div class="card text-center"><div class="card-body"><h6 class="text-muted">Total Spend (SSLCs)</h6><h3 class="text-primary">${formatMoney(totalSpend)}</h3></div></div></div>
...
<div class="card mb-3"><div class="card-header"><strong>Project Detail Lines</strong></div><div class="card-body p-0">
  ${details.length === 0 ? `<p class="text-muted text-center py-3 mb-0">No detail lines. ${p.SCSCommodity_TypeID === 2 ? `<a href="/spend/${p.ProjectID}/sslc">Link SSLCs</a> to add spend.` : ''}</p>` :
    `<div class="table-responsive"><table class="table table-sm table-hover mb-0">
      <thead class="table-light"><tr><th>Commodity</th><th>Code</th><th>Supplier</th><th>Site</th><th>BU</th><th>Region</th><th>Purchase Type</th><th class="text-end">Base</th><th class="text-end">Adjust</th><th class="text-end">Total</th>${p.SCSCommodity_TypeID === 2 && hasPerm(data, 'LinkSSLC') ? '<th></th>' : ''}</tr></thead>
      <tbody>${detailRows}</tbody>
      <tfoot class="table-dark"><tr><td colspan="7" class="text-end fw-bold">Total:</td><td class="text-end">${formatMoney(details.reduce((s,d) => s + (d.BaseSpendVolume||0), 0))}</td><td class="text-end">${formatMoney(details.reduce((s,d) => s + (d.AdjustmentVolume||0), 0))}</td><td class="text-end fw-bold">${formatMoney(totalSpend)}</td>${p.SCSCommodity_TypeID === 2 && hasPerm(data, 'LinkSSLC') ? '<td></td>' : ''}</tr></tfoot>
    </table></div>`}
</div></div>
